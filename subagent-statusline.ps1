...
# Get-ThresholdRole, Test-WideWindow, K, Get-FiniteNumber, Get-PayloadNumber, Format-PayloadText and
# Test-PayloadText, thirteen of them - are copied verbatim from statusline.ps1, and test.ps1 checks
# the same thirteen names for drift, so this list and that one cannot disagree without one of them
# failing. They cannot be shared by dot-sourcing: statusline.ps1 reads stdin to the end and prints as
# it loads, so loading it here would eat this script's payload and print a status line.
[CmdletBinding()]
param()
