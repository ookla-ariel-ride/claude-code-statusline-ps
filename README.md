...
- Fits the terminal width. A line that is too long first loses detail from the cost, limits, cache, context, branch, folder and badges segments, then loses whole segments from the right, so lines stop wrapping in normal use.
...
| `layout` | `one`, `two` | `two` puts model, folder, branch, pr and badges on the first line and context, cache, limits, cost and lines on the second, unless `rows` says otherwise. |
...
| `cost` | `one` | `plain` | model, context, cache, cost, lines, limits |
...
each with its own plan. In rough order: new segments (session clock, links on the folder
