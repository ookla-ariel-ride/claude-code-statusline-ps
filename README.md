...
1. **Detail.** Segments with a short form swap to it, in this order: cost, limits, cache, context,
   branch, folder, badges, clock. The cost delta goes first, the session clock's `api` share last.
...
|---|---|---|---|
| model | <img src="docs/icons/robot.svg" height="18" alt="robot"> `nf-md-robot` | `model.display_name`, `context_window.context_window_size`, `exceeds_200k_tokens`, `context_window.used_percentage`, `rate_limits.five_hour`, `seven_day` | Bold cyan. `display_name` goes through the same guard as the branch and repository names, so a number, a boolean or a blank string is not a name and gives no segment at all, and a right-to-left override or other invisible reordering character in it costs the character rather than the line. On a 1M window `1M` follows the name in a lighter cyan, then a warning triangle when Claude Code reports `exceeds_200k_tokens` as true. The whole segment turns red once the context window or a rate limit reaches the `alarm` percentage, 90 unless the config moves it. The text does not change. This is the one segment that is never shortened and never dropped, which is why the alarm rides on it: at any width, and on either row of layout two, a full context window is still visible as a red line |
