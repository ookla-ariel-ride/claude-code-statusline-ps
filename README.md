...
- Fits the terminal width. A line that is too long first loses detail from the cost, limits, context, branch, folder, badges and clock segments, then loses whole segments from the right, so lines stop wrapping in normal use.
...
| `layout` | `one`, `two` | `two` puts model, folder, branch, pr and badges on the first line and context, limits, cost, clock and lines on the second, unless `rows` says otherwise. |
...
| `cost` | `one` | `plain` | model, context, cost, clock, lines, limits |
| `full` | `two` | `powerline` | all ten |
...
each with its own plan. In rough order: new segments (cache warmth, links on the folder
