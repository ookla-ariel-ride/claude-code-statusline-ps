...
| `layout` | `one`, `two` | `two` puts model, folder, branch, pr and badges on the first line and context, cache, limits, cost, clock and lines on the second, unless `rows` says otherwise. |
...
|---|---|---|
| model, cost, clock, lines, limits, effort, vim | robot, cash, timer, code, tachometer, speedometer, vim | nothing — the name, the `$1.07`, the `1h12m`, the `+156 -23`, the `5h 24%`, the level and the mode say it |
| context | memory | `ctx` |
...
each with its own plan. In rough order: a light palette, and a right-aligned group.
