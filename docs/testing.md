...
Both PNGs are regenerated by hand rather than in CI, and both are reproducible: the script pins the
render's clock with `CLAUDE_STATUSLINE_NOW` (see [docs/diagnostics.md](diagnostics.md)), so the two
countdowns and the wall clock in the two-line shot are the same on every run and in every zone.
Regenerating with no other change leaves `git status` clean; a binary diff on either PNG means the
rendering really moved. The font is the one thing still local to your machine — the images are drawn
with the installed JetBrainsMono NF, so regenerate them with that font installed or the diff will be
the font rather than the change.
