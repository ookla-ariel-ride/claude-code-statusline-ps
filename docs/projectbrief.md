...
- **One segment table, and the config moves what it can.** `Get-SegmentRegistry` is the single list of
  ...
  config, but not the fixed 70 and 90 of a 1M window, which belong to the window size. `quiet` is the
