...
1. New segments: links on the folder and branch (#13). Cache warmth (#2), the hit ratio (#3) and the
   session clock (#8) are done. The first two are deliberately separate: one reads `prompt_cache` for
   whether the cache is alive, the other `context_window.current_usage` for how much of a turn it
   served. Cost per turn (#5) is done and is the first reader of the state file, and the
   agent and session badges (#14) are done; #13 reuses `Format-Link` around the finished branch and
   folder text.
