...
eleven short functions and a drift test. The intended order for the rest:

1. New segments: cache warmth and hit ratio, links on the folder and branch, cost per turn, session
   clock (#2, #3, #5, #8, #13). #5 reads the state file; #13 reuses `Format-Link` around the finished
   branch and folder text. The agent and session badges (#14) are done.
