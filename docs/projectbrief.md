...
| File | Role |
|---|---|
| `statusline.ps1` | Reads JSON on stdin, then `statusline.json` beside it and the project's own copy over that; prints one or two coloured lines fitted to `COLUMNS`. `Read-StdinText` is the read: a `StreamReader` over the standard input handle with UTF-8 named explicitly, because `[Console]::In` decodes at the console's *input* code page and turned every payload field that was not English into mojibake (#80). A byte order mark is consumed rather than carried into the string; `[Console]::InputEncoding` is deliberately never assigned, since that writes a code page this process does not own. |
