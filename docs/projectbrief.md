...
1. New segments: cache warmth and hit ratio, session clock (#2, #3, #8). Cost per turn (#5) is done
   and is the first reader of the state file, the agent and session badges (#14) are done, and the
   folder and branch links (#13) are done, reusing `Format-Link` around the finished text of each.
