...
1. New segments: cache warmth and hit ratio, links on the folder and branch
   (#2, #3, #13). Cost per turn (#5) is done and is the first reader of the state file, the
   agent and session badges (#14) are done, and the session clock (#8) is done; #13 reuses
   `Format-Link` around the finished branch and folder text.
