...
1. New segments: links on the folder and branch, session clock (#8, #13). Cache warmth (#2) and the
   hit ratio (#3) are done and are deliberately separate: one reads `prompt_cache` for whether the
   cache is alive, the other `context_window.current_usage` for how much of a turn it served.
   Cost per turn (#5) is done and is the first reader of the state file, and the
