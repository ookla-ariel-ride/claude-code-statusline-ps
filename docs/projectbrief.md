...
| `<workspace.project_dir>\.claude\statusline.json` | The project's own copy of the same keys, merged over the user file key by key so a repository can pin its layout without changing any other session. Read only when the payload names a project directory that holds it, and not at all when `-Config` names a file. Read as untrusted input: an ordinary file, at most 64 KiB, within a 250 ms deadline. |
...
- **The project file is untrusted input.** It comes with the repository, so `Read-BoundedFileText`
  requires an ordinary file (no link or other reparse point), caps it at 64 KiB and gives the read a
  250 ms deadline, and a stalled handle is left to process exit rather than disposed, since disposing
  it would wait on the same read. `Read-CodePoint` admits a code point only when it draws as one glyph
